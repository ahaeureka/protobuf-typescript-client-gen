...
exports.AxiosClientPlugin = void 0;
// 条件导入，避免浏览器环境报错
let fs;
let Handlebars;
...
const annotations_pb = __importStar(require("./google/api/annotations_pb"));
...
Logger.isDebugEnabled = process.env.DEBUG_PROTOC === 'true';
...
class AxiosClientPlugin {
    ...
    registerHandlebarsHelpers(Handlebars) {
        Handlebars.registerHelper('snake_case', function (str) {
            return str.replace(/([A-Z])/g, '_$1').toLowerCase().replace(/^_/, '');
        });
        Handlebars.registerHelper('camel_case', function (str) {
        ...
        Handlebars.registerHelper('isBodyMethod', function (method) {
    ...
    getOutputFileName(baseFileName) {
    ...
    tryExtractHttpDirectly(options) {
        try {
    ...
    parseHttpRuleArray(httpRuleArray) {
        try {
    ...
    tryExtractFromExtensionMap(options) {
        try {
    ...
    parseHttpRuleFromUnknownField(field) {
        try {
     ...
     * Field number: 50038
     */
    getServiceHttpResponse(serviceOptions) {
        if (!serviceOptions)
            return null;
        try {
     ...
     * Field number: 50033
     */
    getServiceAuthRequired(serviceOptions) {
        if (!serviceOptions)
            return false;
        try {
     ...
     */
    shouldUseHttpResponse(methodOptions, serviceHttpResponse) {
     ...
     */
    isRedirectMethod(methodOptions) {
        if (!methodOptions)
            return false;
        try {
     ...
     */
    getMethodAuthRequired(methodOptions, serviceAuthRequired, methodAuthRule) {
        if (!methodOptions) {
     ...
     * 通用方法：从选项中提取扩展字段
     */
    getExtensionField(options, fieldNumber) {
        if (!options)
            return null;
        try {
            ...
            if (options.wrappers_ && options.wrappers_[fieldNumber.toString()]) {
                ...
                if (wrapper.array && wrapper.array.length > 0) {
                    ...
                    return wrapper.array.find((v) => v !== undefined);
                }
            }
    ...
    generateImports(requiredTypes, localMessageTypes, currentPackage, modelImports, allFiles, options = {}) {
    ...
    processFile(file, allFiles) {
        ...
        file.getServiceList().forEach(service => {
            ...
            service.getMethodList().forEach(method => {
                ...
                if (streamingType == 'client_streaming' || streamingType === 'bidirectional_streaming') {
                    useWebsoket = true;
                }
            });
    ...
    processField(field) {
        ...
        if (fieldLabel === descriptor_pb_1.FieldDescriptorProto.Label.LABEL_OPTIONAL) {
...
exports.AxiosClientPlugin = AxiosClientPlugin;
...
function main() {
    ...
    const request = plugin_pb_1.CodeGeneratorRequest.deserializeBinary(new Uint8Array(input));
    try {
