...
class V1Client {
        ...
        if (isAPIResponse(response.data)) {
            ...
            if (response.data.data !== undefined) {
                ...
                if (userData && typeof userData === 'object' && 'user' in userData) {
                    ...
                    return user_1.User.fromJSON(userData.user);
                }
                console.log('[UserService] Using data directly as User:', userData);
                return user_1.User.fromJSON(userData);
            }
            ...
            return user_1.User.fromJSON(response.data);
        }
    ...
    async upload_avatar(request, headers) {
        // Extract and replace path parameters (supports nested fields like {service.service_name})
        ...
        if (isAPIResponse(response.data)) {
            ...
            if (response.data.data !== undefined) {
                ...
                if (userData && typeof userData === 'object' && 'user' in userData) {
                    console.log('[UserService] Extracting user from nested structure:', userData.user);
                    return user_1.UploadAvatarResponse.fromJSON(userData.user);
                }
                console.log('[UserService] Using data directly as UploadAvatarResponse:', userData);
                return user_1.UploadAvatarResponse.fromJSON(userData);
            }
            // Direct response (APIResponse but without data field - shouldn't normally happen)
            console.log('[UserService] Using response.data directly as UploadAvatarResponse:', response.data);
            return user_1.UploadAvatarResponse.fromJSON(response.data);
        }
        // Not an APIResponse - treat as direct protobuf response
